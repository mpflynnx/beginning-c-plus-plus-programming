...
- [Potential pointer pitfalls](#potential-pointer-pitfalls)
    - [Uninitialised pointers](#uninitialised-pointers)
    - [Dangling pointers](#dangling-pointers)
    - [Checking success of the `new` keyword](#checking-success-of-the-new-keyword)
...
Smart pointers in modern C++ overcome many of the following pitfalls.

#### Uninitialised pointers

```c
int* int_ptr; // pointing anywhere contains garbage
```

If the compiler allows uninitialised, then trying to update the value pointed (as shown below) could overwrite another applications data and crash the program.

```c
*int_ptr = 100;
```
#### Dangling pointers
This is a pointer pointing to released memory, either as the pointer came out of scope after a function call returned or because two pointers pointed to the memory and one pointer released the memory with a delete, leaving the other pointer able to access the data.

#### Checking success of the `new` keyword

If `new` fails an exception is thrown. We can use the exception to handle this. Trying to dereference a null pointer will cause the program to crash.

#### Memory leaks

- Most common problem
- Occurs if the memory is not release using `delete` keyword
- Lost pointers can occur if pointers are created in functions and are deleted upon function return.
- Memory is still used by the program, but not accessible.
- The memory is orphaned or leaked

Smart pointers in modern C++ overcome many of the following pitfulls.


[Return to top](#table-of-contents)

## External References
- [udemy.com | Course content | Section 12: Pointers](https://www.udemy.com/course/beginning-c-plus-plus-programming/learn/lecture/9535524#questions)
- [learncpp.com | The stack and the heap](https://www.learncpp.com/cpp-tutorial/the-stack-and-the-heap/)
